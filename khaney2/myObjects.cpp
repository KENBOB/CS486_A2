...
//draws a Cube/box w/ Texture on Face 'one' & 'five'
FaceList* myCube (float sx, float sy, float sz, UINT texID)
{
   ...
   int n=0;   int  nv=8;   int  nf=12;          // cube: 8 verts, 12 faces  (default)
                       // 2 Triangles that make a QUAD face
...
{  
   ...
   if (strstr(objName,".ply\0")!=NULL) oTYP=PLYfile;    // read PLY file
   if (strstr(objName,".obj\0")!=NULL) oTYP=OBJfile;    // read OBJ file
   ...
   switch ( min(oTYP,1) ) {                          // PLYfile or CUBE ? for now
	  case PLYfile:
		  ...
		  glNewList (LID, GL_COMPILE);               // begin an New List
		     for (int i=0; i<pL->fc; i++) {          // Loop thru each Face; texture ~30% of faces
				if (textureID>0 && 3*i<pL->fc) { glDisable(GL_LIGHTING);  glEnable(GL_TEXTURE_2D);  glBindTexture(GL_TEXTURE_2D,textureID);  }
				glBegin(GL_TRIANGLES);
		        for (int j=0; j<3; j++) {            // ...Loop thru each Triangle vertex
		     	  if (textureID > 0 && 3*i<pL->fc) {  glTexCoord2f(float(j/2),float(j%2));   }       /////    macro TBD ???
				...
				if (textureID>0 && 3*i<pL->fc) { glEnable(GL_LIGHTING);  glDisable(GL_TEXTURE_2D);  }
			 }
		  glEndList();                               // end of List
		  vcopy(ctr,pL->center);                     // set PlyModel Bounding center(x,y,z)
		  RR = sqr(pL->radius)*0.5;                  // set PlyModel Bounding radius^2
...
//	     break;
	  ...
	  case CUBE:
		 ...
		 RR = float(sx*sx +sy*sy +sz*sz)*0.5;        // set Bounding Sphere radius^2
...
void myShaderModel(UINT shaderID) {
	glShadeModel(shaderID+1);               // DBG: testing
};
