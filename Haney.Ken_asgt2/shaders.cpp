...
CShader::CShader()                             // class constructor
{
	bLoaded = false;
}
...
bool CShader::loadShader(string sFile, int a_iType)
{
	...
	if (iStatus == GL_FALSE) return false;
	iType = a_iType;
	bLoaded = true;
	return 1;
}
...
/*---------------------------------------------*/
UINT CShader::getShaderID() {  return uiShader;  }
...
/*---------------------------------------------*/
CShaderProgram::CShaderProgram() {  bLinked = false;  }
...
Param:	sShader - shader to add
Result:	Adds a shader (like source file) to a program, but only compiles once.
/*---------------------------------------------*/
...
/*---------------------------------------------*/
void CShaderProgram::useProgram() {  if (bLinked) glUseProgram(uiProgram);  }

/*-----------------------------------------------
Name:		getProgramID
Result:	Returns OpenGL generated shader program ID.
/*---------------------------------------------*/
UINT CShaderProgram::getProgramID()  {  return uiProgram;  }

/*-----------------------------------------------
Name:		uniformSetters
Params:	    various, as needed
Result:	these functions set many common uniform variables.
/*---------------------------------------------*/
